<?php
session_start();
?>

<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <link rel="stylesheet" href="stylesheet/style.css" />
    <link href="https://fonts.googleapis.com/css2?family=Alfa+Slab+One&family=Architects+Daughter&family=Paytone+One&family=Yeseva+One&display=swap" rel="stylesheet">
    <title>Event Planner</title>
</head>

<body>
    <nav class="nav">
        <div class="container">
            <h1 class="logo" style='font-family: "Paytone One", sans-serif; font-size: 30px'><a href="home.php">Event <span style="color:#bb4794" class="plan">Planner</span></a></h1>
            <ul>
                <li><a href="homelogged.php">Home</a></li>
                <li><a href="gallery.php">Gallery</a></li>
                <li><a href="services.php">Services</a></li>
                <li><a href="feedback.php">Feedback</a></li>
                <li><a href="login.php"><?php echo "Welcome " . $_SESSION['username'] ?></a></li>
            </ul>
        </div>
    </nav>

</body>

</html>